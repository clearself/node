 ...
 * 获取用户已经领取的优惠券
 */
router.get('/record',tokenMiddlWare, async (req, res) => {
  try {
    const { userId } = req.api_user;
 ...
 * 获取用户已经领取的具体某一个优惠券列表
 */
router.get('/received',tokenMiddlWare, async (req, res) => {
  try {
 ...
 * 获取优惠券详情
 */
router.get('/detail',tokenMiddlWare, async (req, res) => {
  try {
    const { id } = req.query;
...
* 立即获取优惠券
*/
router.post('/receive',tokenMiddlWare, async (req, res) => {
  try {
