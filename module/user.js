...
router.get('/info',tokenMiddlWare,async (req, res) => {
  try {
    const { id } = req.query;
 ...
 * 修改头像
 */
router.post('/changeUserHeadpic',tokenMiddlWare, multer.array('file'), async (req, res) => {
  try {
    const cwd = process.cwd();
    const file = req.files[0];
    const userId = req.api_user.userId;
...
* 修改用户名
*/
router.post('/changeUserName',tokenMiddlWare, async (req, res) => {
  try {
    const msg = req.body;
    const userId = req.api_user.userId;
...
* 修改性别
*/
router.post('/changeUserSex',tokenMiddlWare, async (req, res) => {
  try {
    const msg = req.body;
    const userId = req.api_user.userId;
    const sex = msg.sex;
 ...
 * 重置密码
 */
router.post('/resetPassword',tokenMiddlWare, async (req, res) => {
  try {
